...
describe('render change password page', () => {
	...
	it('error if auth completeNewPassword failed', async () => {
		Auth.completeNewPassword = jest.fn().mockImplementation((value: string) => {
	...
	it('error if UserService getUsers failed', async () => {
		Auth.completeNewPassword = jest.fn().mockImplementation((temp: any, value: string) => {
			return {
				signInUserSession: {
					idToken: {
						payload: {
		...
		UserService.getUsers = jest.fn().mockImplementation((value: object) => {
	...
	it('redirect on "/" if user found', async () => {
		...
		EventService.getNextEvent = jest.fn().mockImplementation((value: object) => {
			return [];
		});
		NewArrivalsService.getNewArrivals = jest.fn().mockImplementation((value: object) => {
			return [];
		});
		const button = screen.getByText(/change-password.connection/i);
		expect(button).not.toHaveAttribute('disabled');
	...
	it('error if UserService creatUser failed', async () => {
		...
		UserService.creatUser = jest.fn().mockImplementation((value: object) => {
			throw new Error('Failed');
		});
		const button = screen.getByText(/change-password.connection/i);
		expect(button).not.toHaveAttribute('disabled');
